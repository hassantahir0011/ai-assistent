    ...
    try {
        ...
        switch (postData.request_type) {
            ...
            case "disputes/update":
                return addUpdateDisputes(postData);
                break;
            case "subscription_contracts/create":
                return addSubscriptionContractRows(postData);
                break;
            case "subscription_contracts/update":
                return updateSubscriptionContractRows(postData);
                break;
...
function addUpdateDisputes(postData) {
    try {
        var sheet = getSheetByName('disputes');
        if (!sheet) return ContentService.createTextOutput("No sheet having name 'disputes' found in spreadsheet.Please create new sheet with this name and keep header fields described into docs.");
        var data = sheet.getDataRange().getValues();
        var columnNo = 1;
        var foundRowInSheet = false;
        for (var i = 1; i <= data.length; i++) {
            ...
            if (typeof currentSheetRow != 'undefined' && postData.id && currentSheetRow[0] == postData.id) {
                ...
                return ContentService.createTextOutput("dispute row updated successfully.");
        ...
        if (!foundRowInSheet) {
            ...
            return ContentService.createTextOutput("disputes row added successfully.");
        }
    } catch (err) {
        return ContentService.createTextOutput(err.message);
    }
}
    function updateSubscriptionContractRows(postData) {
        try {
            var sheet = getSheetByName('subscription_contracts');
            if (!sheet) return ContentService.createTextOutput("No sheet having name 'subscription_contracts' found in spreadsheet.");
            var foundRowInSheet = false;
            var data = sheet.getDataRange().getValues();
            var newRow = 0;
            for (var i = 1; i <= data.length; i++) {
                ...
                if (typeof currentSheetRow !== 'undefined' && currentSheetRow[0] == postData.id) {
                    foundRowInSheet = true;
                        ...
                        newRow = i + 1;
                        sheet.getRange(newRow, 1).setValue(postData.id);
                        sheet.getRange(newRow, 2).setValue(postData.currency_code);
                        sheet.getRange(newRow, 3).setValue(postData.customer_id);
                        sheet.getRange(newRow, 4).setValue(postData.status);
                        sheet.getRange(newRow, 5).setValue(postData.origin_order_id);
                    if (postData.billing_policy){
                        sheet.getRange(newRow, 6).setValue(postData.billing_policy.interval || '');
                        sheet.getRange(newRow, 7).setValue(postData.billing_policy.interval_count || 0);
                        sheet.getRange(newRow, 8).setValue(postData.billing_policy.min_cycles || 0);
                        sheet.getRange(newRow, 9).setValue(postData.billing_policy.max_cycles || 0);
                    }
                    if (postData.delivery_policy){
                        sheet.getRange(newRow, 10).setValue(postData.delivery_policy.interval || '');
                        sheet.getRange(newRow, 11).setValue(postData.delivery_policy.interval_count || 0);

                    }
                    return ContentService.createTextOutput("Subscription Contract updated successfully.");
                    SpreadsheetApp.flush();
                }
            }
            if (!foundRowInSheet)
    ...
    function addSubscriptionContractRows(postData) {
        try {
            var sheet = getSheetByName('subscription_contracts');
            if (!sheet) return ContentService.createTextOutput("No sheet having name 'subscription_contracts' found in spreadsheet.");
            var lastRow = Math.max(sheet.getLastRow(), 1);
            ...
            SpreadsheetApp.flush();
            return ContentService.createTextOutput("Subscription Contract row added successfully.");
        } catch (err) {
            return ContentService.createTextOutput(err.message);
        }
    }
